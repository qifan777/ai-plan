...
export const useTableHelper = <T extends object, E>(
  ...
  // 请求分页数据
  const loadTableData = async (request: Partial<QueryRequest<T>>) => {
    queryRequest.value = {
      ...queryRequest.value,
    ...
    loading.value = true
    const res = await queryApi.apply(object, [{ body: queryRequest.value }])
  ...
  // 重新请求分页数据，pageNum=1
  const reloadTableData = async (queryRequest: Partial<QueryRequest<T>> = { pageNum: 1 }) => {
    handleSelectChange([])
    table.value?.clearSelection()
    await loadTableData(queryRequest)
  }
  // 获取表格选中的数据
  ...
  const handleSortChange = async ({ prop, order }: SortChange) => {
