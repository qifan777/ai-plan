...
import type { AiMessageDto } from '@/apis/__generated/model/dto'
import { Delete } from '@element-plus/icons-vue'
import { useTableHelper } from '@/components/base/table/table-helper'

type AiMessageScope = Scope<AiMessageDto['AiMessageRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const aiMessageTableHelper = inject(
  'aiMessageTableHelper',
  useTableHelper(api.aiMessageForAdminController.query, api.aiMessageForAdminController, {}),
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.aiMessageForAdminController.delete({ body: ids }).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="120"
      >
        <template v-slot:default="{ row }: AiMessageScope">
          {{ row.type }}
        </template>
      </el-table-column>
      <el-table-column
