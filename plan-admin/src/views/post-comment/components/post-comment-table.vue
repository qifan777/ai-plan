...
import type { PostCommentDto } from '@/apis/__generated/model/dto'
import { Delete } from '@element-plus/icons-vue'
import { useTableHelper } from '@/components/base/table/table-helper'

type PostCommentScope = Scope<PostCommentDto['PostCommentRepository/COMPLEX_FETCHER_FOR_ADMIN']>
const postCommentTableHelper = inject(
  'postCommentTableHelper',
  useTableHelper(api.postCommentForAdminController.query, api.postCommentForAdminController, {})
)
const {
  loadTableData,
  handleSortChange,
...
const handleDelete = (ids: string[]) => {
  ElMessageBox.confirm('此操作将删除数据且无法恢复, 是否继续?', '警告', {
    ...
    api.postCommentForAdminController.delete({ body: ids }).then((res) => {
  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        width="120"
      >
        <template v-slot:default="{ row }: PostCommentScope">
          {{ row.parent?.content }}
        </template>
      </el-table-column>
      <el-table-column
